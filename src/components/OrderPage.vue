<template>
    <div class="mT20 mB30">
        ...
        <el-select class="float-right mB10" v-model="orderTableCols" multiple collapse-tags>
            <el-option v-for="(col, i) in orderTableColsSet" :value="col" :key="i">{{ col }}</el-option>
        </el-select>
        <el-table ref="orderListTable" class="dashboardCard-shadow" :stripe="true" :data="orderList">
            ...
            <el-table-column v-if="orderTableCols.includes('status')" label="Status">
                <template slot-scope="scope">
                    <el-select class="statusSelect" :class="statusSelect(scope.row.status)" :value="orderList[scope.$index].status" @change="changeStatus($event, scope.$index)" placeholder="選擇訂單狀態">
